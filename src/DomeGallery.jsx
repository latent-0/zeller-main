...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const ovlRel  = { left: currentRect.left - rootRect.left,  top: currentRect.top  - rootRect.top,  width: currentRect.width, height: currentRect.height };

      const animDiv = document.createElement('div');
      animDiv.className = 'enlarge-closing';
      animDiv.style.cssText = `position:absolute;left:${ovlRel.left}px;top:${ovlRel.top}px;width:${ovlRel.width}px;height:${ovlRel.height}px;z-index:9999;border-radius:var(--enlarge-radius,32px);overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.35);transition:all ${enlargeTransitionMs}ms ease-out;pointer-events:none;margin:0;transform:none;`;
      const origImg = overlay.querySelector('img');
      if (origImg) { const img = origImg.cloneNode(); img.style.cssText = 'width:100%;height:100%;object-fit:cover;'; animDiv.appendChild(img); }
      overlay.remove();
  ...
  const openItemFromElement = useCallback(el => {
    ...
    if (openedImageWidth || openedImageHeight) {
      const onFirstEnd = ev => {
        ...
        const tmpW = openedImageWidth  || `${frameR.width}px`;
        const tmpH = openedImageHeight || `${frameR.height}px`;
        overlay.style.width  = tmpW;
        overlay.style.height = tmpH;
        const newR = overlay.getBoundingClientRect();
        overlay.style.width  = `${frameR.width}px`;
        overlay.style.height = `${frameR.height}px`;
        void overlay.offsetWidth;
        overlay.style.transition = `left ${enlargeTransitionMs}ms ease,top ${enlargeTransitionMs}ms ease,width ${enlargeTransitionMs}ms ease,height ${enlargeTransitionMs}ms ease`;
        const cL = frameR.left - mainR.left + (frameR.width  - newR.width)  / 2;
        const cT = frameR.top  - mainR.top  + (frameR.height - newR.height) / 2;
        requestAnimationFrame(() => {
        ...
        const cleanupSecond = () => { overlay.removeEventListener('transitionend', cleanupSecond); overlay.style.transition = prevTrans; };
